...
def evaluate(
    ...
    """
    server.model_loader = ModelLoader(model_type, is_trained=True)
    ...
    if output_path.exists():
        ...
        if isinstance(existing, list):
            existing = [
                ...
                if entry.get("model_type") != model_type.name
            ]
...
def visualize(
    ...
    report_summary = (
        df.groupby(["report_type", "model_type"])["accuracy"].mean().unstack()
    )
    ...
    type_summary = df.groupby(["type", "model_type"])["accuracy"].mean().unstack()
    ...
    metrics_df = pd.DataFrame(metrics).set_index("model_type")
